...
public class RoadModule extends ConfigurableWorldModule {
    ...
    public void applyTo(MapData grid) {
        ...
        for (MapNode node : grid.getMapNodes()) {
            ...
            if (connectedRoads.size() > 2) {
                ...
                if (road1.getWidth() != road2.getWidth() /* TODO: || lane layouts not identical */) {
    ...
    /**
     * determines surface for a junction or connector/crossing. If the node has
     * an explicit surface tag, this is evaluated. Otherwise, the result depends
     * on the surface values of adjacent roads.
     */
    ...
    /**
     * find matching lane pairs (lanes that can be connected at a junction or
     * connector)
     */
    ...
    /**
     * builds lane connections at a junction of just oneway roads. Intended to
     * handle motorway merges and splits well. Inbound and outbound roads must
     * not be mixed, but build two separate continuous blocks instead.
     *
    ...
    /**
     * determines connected lanes at a junction, crossing or connector for a
     * pair of two of the junction's roads. Only connections between the left
     * part of road1 with the right part of road2 will be taken into account.
     */
    ...
    public static class Road
        ...
        //TODO TOMMY, usato per poter dare una dimensione alla via
        ...
        private double calculateFallbackWidth() {
            ...
            for (Lane lane : laneLayout.getLanesLeftToRight()) {
                ...
                if (lane.type == VEHICLE_LANE && ignoreVehicleLanes) {
                    continue;
                }
    ...
    private static class LaneLayout {
        ...
        /**
         * calculates and sets all lane attributes that are not known during
         * lane creation
         */
     ...
     *
     * Field values depend on neighboring lanes and are therefore calculated and
     * defined in two phases. Results are then set using
    ...
    private static final class Lane implements RenderableToAllTargets {
        ...
        /**
         * provides access to the first and last node of the lane's left and
         * right border
         */
    ...
    private static class LaneConnection implements RenderableToAllTargets {
        ...
        /**
         * returns the outline of this connection. For determining the total
         * terrain covered by junctions and connectors.
         */
    ...
    /**
     * a type of lanes. Determines visual appearance, and contains the
     * intelligence for evaluating type-specific tags.
     */
    ...
    /**
     * a texture coordinate function for arrow road markings on turn lanes. Has
     * special features including centering the arrow, placing it at an offset
     * from the end of the road, and taking available space into account.
     *
