...
public final class WorldModuleBillboardUtil {
     ...
     * and back side, intersecting orthogonally. Each billboard consists of two
     * halves, separated at the billboard's intersection axis, to allow sorting
     * of primitives for transparent rendering.
     */
    ...
    private static final List<List<VectorXZ>> buildBillboardTexCoordLists(
        ...
        if (right) {
            ...
            if (mirrored) {
                if (copies <= 3) {
                    return BILLBOARD_RIGHT_TEX_COORDS_MIRRORED_COPIES[copies];
                } else {
                    return nCopies(copies, BILLBOARD_RIGHT_TEX_COORDS_MIRRORED);
                }
            } else {
                if (copies <= 3) {
                    return BILLBOARD_RIGHT_TEX_COORDS_COPIES[copies];
                } else {
            ...
            if (mirrored) {
                if (copies <= 3) {
                    return BILLBOARD_LEFT_TEX_COORDS_MIRRORED_COPIES[copies];
                } else {
                    return nCopies(copies, BILLBOARD_LEFT_TEX_COORDS_MIRRORED);
                }
            } else {
                if (copies <= 3) {
                    return BILLBOARD_LEFT_TEX_COORDS_COPIES[copies];
                } else {
