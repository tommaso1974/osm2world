...
/**
 * class that will calculate the information for those {@link MapElement}s that
 * implement an interface from org.o3dw.representation.network and set it.
...
public class NetworkCalculator {
    ...
    private static void calculateConnectorNodeEffects(
        ...
        if (!inbound1) {
            inVector = inVector.invert();
        }
        if (inbound2) {
            outVector = outVector.invert();
        }
