...
public abstract class JunctionNodeWorldObject extends OutlineNodeWorldObject {
     ...
     *
     * Cut information will not be created for all way/area segments. The lists
     * can therefore contain null entries.
     *
     * @param cutCenters centers of the cuts to each; indices are the same as
     * for the GridNode's {@link MapNode#getConnectedSegments()}
     * @param cutVectors vectors describing indicating the cut line, pointing to
     * the right from the node's pov; for indices see junctionCutCenters
     * @param widths widths of the junction cut; for indices see
     * junctionCutCenters
     */
...
//	}
    ...
    /**
     * calculates the left or right node of a cut (Only available if junction
     * information for this representation has been provided using
     * {@link #setInformation(List, List, List)}).
     *
     ...
     *
     * The from and to indices refer to the list returned by the underlying
     * {@link MapNode}'s {@link MapNode#getConnectedSegments()} method.
     */
    ...
    /**
     * throws an IllegalStateException if information hasn't been provided by a
