...
public abstract class VisibleConnectorNodeWorldObject
    ...
    /**
     * returns the length required by this node representation. Adjacent lines
     * will be pushed back accordingly.
     *
     * If this is 0, this has the same effect as an invisible connector node
     * (adjacent line representations directly touching each other). Examples
     * where non-zero values are needed include crossings at nodes in roads.
     *
     * Needs to be provided by the implementing class before the calculation in
     * {@link NetworkCalculator} starts.
     */
    abstract public float getLength();

    /**
     * sets the results of {@link NetworkCalculator}'s calculations. Most
     * methods in this class cannot be used until this method has provided the
    ...
    protected List<VectorXZ> getOutlineXZ(int from, int to) {
        ...
        List<VectorXZ> outline = new ArrayList<>();
