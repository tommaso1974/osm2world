...
public abstract class AbstractNetworkWaySegmentWorldObject
    ...
    /**
     * calculates centerline and outlines, along with their connectors TODO:
     * perform before construction, to simplify the object and avoid
     * {@link #broken}
     */
        ...
        { /* calculate centerline */
            ...
            for (MapOverlap<?, ?> overlap : segment.getOverlaps()) {
                ...
                if (overlap instanceof MapIntersectionWW) {
                    ...
                    if (!(overlap.getOther(segment).getPrimaryRepresentation() instanceof AbstractAreaWorldObject)) {
                        continue;
                    }
    ...
    /**
     * implementation of {@link WorldObject#getGroundState()}. This version
     * checks for bridge and tunnel tags to make the decision. If that is not
     * desired, subclasses may override the method.
     */
    @Override
    ...
    /**
     * returns a sequence of node running along the center of the line from
     * start to end (each with offset). Uses the
     * {@link WaySegmentElevationProfile} for adding elevation information.
     */
    ...
    /**
     * 3d version of {@link #getCenterlineXZ()}. Only available after elevation
     * calculation.
     */
    ...
    /**
     * Variant of {@link #getOutline(boolean)}. This one is already available
     * before elevation calculation.
     */
    ...
    /**
     * provides the left or right border (a line at an appropriate distance from
     * the center line), taking into account cut vectors, offsets and elevation
     * information. Available after cut vectors, offsets and elevation
     * information have been calculated.
     *
     * Left and right border have the same number of nodes as the elevation
     * profile's {@link WaySegmentElevationProfile#getPointsWithEle()}. //TODO:
     * compatible with future offset/clearing influences?
     */
    ...
    /**
     * checks whether this segment has a broken outline. That can happen e.g. if
     * it lies between two junctions that are too close together.
     */
