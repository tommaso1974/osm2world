...
public class OSMToMapDataConverter {
    ...
    public MapData createMapData(OSMData osmData) throws IOException {
        ...
        final List<MapWaySegment> mapWaySegs = new ArrayList<>();
    ...
    /**
     * creates {@link MapElement}s based on OSM data from an
     * {@link OSMFileDataSource} and adds them to collections
     */
    ...
    private static void calculateIntersectionsInMapData(MapData mapData) {
        ...
        for (MapElement e1 : mapData.getMapElements()) {
            ...
            for (MapElement e2 : nearbyElements) {
                ...
                //Gestisce la sovrapposizione fra vari elementi
    ...
    /**
     * adds the overlap between two {@link MapElement}s to both, if it exists.
     * It calls the appropriate subtype-specific addOverlapBetween method
     */
    private static void addOverlapBetween(MapElement e1, MapElement e2) {

        if (e1 instanceof MapWaySegment && e2 instanceof MapWaySegment) {
            addOverlapBetween((MapWaySegment) e1, (MapWaySegment) e2);
        } else if (e1 instanceof MapWaySegment && e2 instanceof MapArea) {
            addOverlapBetween((MapWaySegment) e1, (MapArea) e2);
        } else if (e1 instanceof MapArea && e2 instanceof MapWaySegment) {
            addOverlapBetween((MapWaySegment) e2, (MapArea) e1);
        } else if (e1 instanceof MapArea && e2 instanceof MapArea) {
            addOverlapBetween((MapArea) e1, (MapArea) e2);
        } else if (e1 instanceof MapNode && e2 instanceof MapArea) {
            addOverlapBetween((MapNode) e1, (MapArea) e2);
        } else if (e1 instanceof MapArea && e2 instanceof MapNode) {
            addOverlapBetween((MapNode) e2, (MapArea) e1);
        }
    }

    /**
     * adds the overlap between two {@link MapWaySegment}s to both, if it exists
     */
    private static void addOverlapBetween(MapWaySegment line1, MapWaySegment line2) {
    ...
    /**
     * adds the overlap between a {@link MapWaySegment} and a {@link MapArea} to
     * both, if it exists
     */
    ...
    private static void addOverlapBetween(MapNode node, MapArea area) {
        ...
        if (area.getPolygon().contains(node.getPos())) {
            ...
            /* add the overlap */
            MapOverlapNA newOverlap = new MapOverlapNA(node, area, MapOverlapType.CONTAIN);
            area.addOverlap(newOverlap);
