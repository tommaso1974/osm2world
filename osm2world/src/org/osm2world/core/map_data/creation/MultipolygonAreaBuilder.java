...
/**
 * utility class for creating areas from multipolygon relations, including those
 * with non-closed member ways.
 *
 * Known Limitations:<ul>
 * <li>This cannot reliably handle touching inner rings consisting of non-closed
 * ways.</li>
 * <li>Closed touching rings will not break the calculation, but are represented
 * as multiple touching holes.</li>
 * </ul>
 */
final class MultipolygonAreaBuilder {
    ...
    /**
     * Creates areas for a multipolygon relation. Also adds this area to the
     * adjacent nodes using {@link MapNode#addAdjacentArea(MapArea)}.
     *
     * @param relation the multipolygon relation
     * @param nodeMap map from {@link OSMNode}s to {@link MapNode}s
     *
     * @return constructed area(s), multiple areas will be created if there is
     * more than one outer ring. Empty for invalid multipolygons.
     */
    ...
    private static final boolean isSimpleMultipolygon(OSMRelation relation) {
        ...
        for (OSMMember member : relation.relationMembers) {
            ...
            if ("outer".equals(member.role) && member.member instanceof OSMWay) {
                numberOuters += 1;
            }

            if (("outer".equals(member.role) || "inner".equals(member.role)) && member.member instanceof OSMWay) {
    ...
    /**
     * handles the common simple case with only one outer way. Expected to be
     ...
     * @param nodeMap
     */
    private static final Collection<MapArea> createAreasForSimpleMultipolygon(OSMRelation relation, Map<OSMNode, MapNode> nodeMap) {
        ...
        for (OSMMember member : relation.relationMembers) {
            if (member.member instanceof OSMWay) {
                ...
                if ("inner".equals(member.role)) {
                    ...
                    List<MapNode> hole = new ArrayList<>(way.nodes.size());
    ...
    private static final Collection<MapArea> createAreasForAdvancedMultipolygon(OSMRelation relation, Map<OSMNode, MapNode> nodeMap) {

        List<NodeSequence> innersAndOuters = new ArrayList<>();
     ...
     */
    private static final List<Ring> buildRings(List<NodeSequence> sequences) {

        List<Ring> closedRings = new ArrayList<>();
        ...
        while (sequences.size() > 0) {
            ...
            if (currentRing != null && currentRing.isClosed()) {
                try {
                    ...
                    throw new InvalidGeometryException(String.format("self-intersecting ring (with %d nodes)", currentRing.size() - 1), e);
                }
            }
        }
    ...
    private static final Collection<MapArea> buildPolygonsFromRings(
        ...
        Collection<MapArea> finishedPolygons = new ArrayList<>(rings.size() / 2);
        ...
        while (!rings.isEmpty()) {
            ...
            for (Ring candidate : rings) {
                ...
                for (Ring otherRing : rings) {
                    if (otherRing != candidate && otherRing.containsRing(candidate)) {
            ...
            Collection<Ring> innerRings = new ArrayList<>();
            ...
            List<List<MapNode>> holes = new ArrayList<>(innerRings.size());
            List<SimplePolygonXZ> holesXZ = new ArrayList<>(innerRings.size());
    ...
    /**
     * turns all coastline ways into {@link MapArea}s based on an artificial
     * natural=water multipolygon relation.
     *
     * It relies on the direction-dependent drawing of coastlines. If coastlines
     * are incomplete, then it is attempted to connect them to proper rings. One
     * assumption being used is that they are complete within the file's bounds.
     *
     * It cannot distinguish between water and land tiles if there is no
     * coastline at all (it will then guess based on the tags being used), but
     * should be able to handle all other cases.
     */
    public static final Collection<MapArea> createAreasForCoastlines(
        ...
        List<OSMWay> coastlineWays = new ArrayList<>();
        ...
        if (fileBoundary != null) {
            ...
            List<NodeSequence> origCoastlines = new ArrayList<>();
            ...
            for (final LineSegmentXZ side : getSidesClockwise(fileBoundary)) {
                ...
                Collections.sort(intersectionsSide, new Comparator<NodeOnBBox>() {
                    @Override
                    public int compare(NodeOnBBox n1, NodeOnBBox n2) {
                ...
                if (closedRings != null) {
                    ...
                    if (hasIsland || isProbablySeaTile(osmData)) {
                        ...
                        for (VectorXZ pos : fileBoundary.polygonXZ().getVertices()) {
                            boundaryRing.add(createFakeMapNode(pos, ++highestNodeId, osmData, nodeMap, mapNodes));
                        }
    ...
    private static final List<LineSegmentXZ> getSidesClockwise(AxisAlignedBoundingBoxXZ fileBoundary) {
    ...
    private static MapNode createFakeMapNode(VectorXZ pos, long nodeId,
        ...
        OSMNode osmNode = new OSMNode(NaN, NaN, COASTLINE_NODE_TAGS, nodeId + 1);
    ...
    private static final class NodeSequence extends ArrayList<MapNode> {
        ...
        /**
         * tries to add another sequence onto the start or end of this one. If
         * it succeeds, the other sequence may also be modified and should be
         * considered "spent".
         */
    ...
    private static final class Ring implements IntersectionTestObject {
        ...
        public AxisAlignedBoundingBoxXZ getAxisAlignedBoundingBoxXZ() {
            ...
            for (MapNode n : closedNodeSequence) {
                minX = min(minX, n.getPos().x);
                minZ = min(minZ, n.getPos().z);
                maxX = max(maxX, n.getPos().x);
                maxZ = max(maxZ, n.getPos().z);
            }
    ...
    private static class NodeOnBBox {
        ...
        public String toString() {
            return "(" + outgoingIntersection + ", " + node.getOsmNode().id + "@" + node.getPos() + ")";
        }
