...
/**
 * grid representation of an OSM node, references inbound and outbound
 * {@link MapWaySegment}s. For each OSM node, one GridNode will be created.
 */
public class MapNode implements MapElement {
    ...
    /**
     * returns those connected lines that end here. Sorting is as for
    ...
    /**
     * returns all way segments connected with this node. They will be sorted
     * according to the clockwise (seen from above) angle between the vector
     * "this node -> other node of the segment" and the positive x direction.
     */
