 ...
 *
 * @param <R> subtype of {@link Renderable} designed for visualization with this
 * target
 */
public interface Target<R extends Renderable> {
    ...
    /**
     * renders a renderable object to this target. Usually, this means calling a
     * "renderTo" method on that renderable, with this target as a parameter.
     */
    void render(R renderable);

    /**
     * announces the begin of the draw* calls for a {@link WorldObject}. This
     * allows targets to group them, if desired. Otherwise, this can be ignored.
     */
     ...
     * draws triangles.
     *
     * @param texCoordLists one texture coordinate list per texture. Each must
     * have three coordinates per triangle. Can be null if no texturing
     ...
     * @param texCoordLists one texture coordinate list per texture. Each must
     * have the same length as the "vs" parameter. Can be null if no texturing
     * information is available.
     */
    ...
    /**
     * draws a column with outward-facing polygons around a point. A column is a
     * polygon with > 3 corners extruded upwards.
     *
     * The implementation may decide to reduce the number of corners in order to
     * improve performance (or make rendering possible when a perfect cylinder
     * isn't supported).
     *
     * @param corners number of corners; null creates a cylinder for
     * radiusBottom == radiusTop or (truncated) cone otherwise
     */
    ...
    /**
     * gives the target the chance to perform finish/cleanup operations after
