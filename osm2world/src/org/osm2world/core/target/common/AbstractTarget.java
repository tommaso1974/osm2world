...
/**
 * superclass for {@link Target} implementations that defines some of the
 * required methods using others. Extending it reduces the number of methods
 * that have to be provided by the implementation
 */
public abstract class AbstractTarget<R extends Renderable>
    ...
    public void drawColumn(Material material, Integer corners, VectorXYZ base,
        ...
        if (drawBottom) {
            drawTriangleFan(material, bottomFan,
                    texCoordLists(bottomFan, material, GLOBAL_X_Z));
        }

        if (drawTop) {
            drawTriangleFan(material, topFan,
                    texCoordLists(bottomFan, material, GLOBAL_X_Z));
        }
