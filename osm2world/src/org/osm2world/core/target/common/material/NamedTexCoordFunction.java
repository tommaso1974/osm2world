...
/**
 * several useful {@link TexCoordFunction} implementations. They can be
 * referenced by name in style definition files. {@link Target}s may also
 * provide special implementations for them (e.g. as a specialized shader).
 */
public enum NamedTexCoordFunction implements TexCoordFunction {
     ...
     * to place a texture. This function works for all geometries, but steep
     * inclines or even vertical walls produce odd-looking results.
     */
    GLOBAL_X_Z,
    /**
     * like {@link #GLOBAL_X_Z}, but uses y instead of z dimension. Better
     * suited for certain vertical surfaces.
     */
    GLOBAL_X_Y,
    /**
     * creates texture coordinates for individual triangles that orient the
     * texture based on each triangle's downward slope.
     *
     ...
     *
     * This only works for vertices forming a triangle strip, alternating
     ...
     * between upper and lower vertex).
     *
     * Most commonly used to texture a rectangle represented as a triangle strip
     * with 2 triangles.
     */
    STRIP_FIT;
