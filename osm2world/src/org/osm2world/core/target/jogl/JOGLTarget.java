...
public final class JOGLTarget extends PrimitiveTarget<RenderableToJOGL> {
     ...
     *
     * @param renderingParameters global parameters for rendering; see
     * {@link #setRenderingParameters(JOGLRenderingParameters)}
     * @param globalLightingParameters global parameters for lighting; see
     * {@link #setGlobalLightingParameters(GlobalLightingParameters)}
     */
    ...
    /**
     * similar to {@link #render(Camera, Projection)}, but allows rendering only
     * a part of the "normal" image. For example, with xStart=0, xEnd=0.5,
     * yStart=0 and yEnd=1, only the left half of the full image will be
     * rendered, but it will be stretched to cover the available space.
     *
    ...
    /**
     * similar to {@link #applyProjectionMatrices(GL2, Projection)}, but allows
     * rendering only a part of the "normal" image.
     */
    ...
    static final int getGLTextureConstant(int textureNumber) {
        switch (textureNumber) {
            ...
            case 3:
                return GL_TEXTURE3;
            default:
                throw new Error("programming error: unhandled texture number");
        }
    }
     ...
     *
     * @param clearColor background color before rendering any primitives; null
     * uses a previously defined clear color
     */
