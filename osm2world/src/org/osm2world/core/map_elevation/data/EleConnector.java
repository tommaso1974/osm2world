 ...
 *
 * This class is a core concept of elevation calculation: Because there is no
 * injective mapping from xz coords to elevation (that would not allow for
 * bridges etc.), we need to represent whether points are supposed to be "the
 * same" with regards to elevation in some other manner. Moreover, not all such
 * points correspond to {@link MapNode}s. Thus, this purpose is served by
 * EleConnectors.
 */
public class EleConnector {
    ...
    /**
     * indicates whether this connector should be connected to the terrain, or
     * is instead above or below the terrain
     */
    ...
    /**
     * assigns the elevation that has been calculated for this connector. Only
     * for use by an {@link ElevationCalculator}.
     *
     ...
     *
     * The elevation, and therefore this {@link VectorXYZ}, is the only property
     * which changes (exactly once) over the lifetime of an
     * {@link EleConnector}: It is null before elevation calculation, and
     * assigned its ultimate value afterwards.
     */
    ...
    /**
     * returns true if this connector is to be joined with the other one. It is
     * possible that connectors are joined even if this method returns false -
     * that can happen when they are both joined to a third connector.
     */
